...
def check_hosted_assets(data, errors, warnings):
    ...
    # Check for generator strings from sources that prohibit redistribution.
    ...
    for field in ("footprint", "symbol"):
        ...
        for signature, source_name in PROHIBITED_GENERATORS.items():
            if signature in content:
                errors.append(
                    f"{field} file '{path}' was generated by {source_name}, which "
                    f"prohibits redistribution — replace it with a KiCad official "
                    f"library file or one drawn from the manufacturer datasheet"
                )
                break
