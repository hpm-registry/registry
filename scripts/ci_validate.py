...
# Generator strings that identify files from sources that prohibit redistribution.
# Checked case-insensitively against the full file content.
...
def check_asset_generators(data, errors):
    """Detect footprint/symbol files stamped by sources that prohibit redistribution."""
    for field in ("footprint", "symbol"):
        block = data.get(field)
        if not isinstance(block, dict):
            continue
        ...
        asset = REPO_ROOT / path
        if not asset.is_file():
            continue  # missing file already caught by check_hosted_assets

        content = asset.read_text(errors="replace").lower()
        for signature, source_name in PROHIBITED_GENERATORS.items():
            if signature in content:
                errors.append(
                    f"{field} file '{path}' was generated by {source_name}, which prohibits "
                    f"redistribution — replace it with a KiCad official library file or one "
                    f"drawn from the manufacturer datasheet"
                )
...
def main():
    ...
    for path in files:
        ...
        check_asset_generators(data, errors)
