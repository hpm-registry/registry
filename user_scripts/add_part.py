...
def check_prohibited_files(footprint_paths, symbol_path):
    """Scan footprint and symbol files for prohibited generator signatures.

    Exits immediately with a clear explanation if any are found.
    Called before prompting the contributor for any information.
    """
    prohibited = []
    for f in [symbol_path] + list(footprint_paths):
        content = Path(f).read_text(errors="replace").lower()
        for signature, source_name in PROHIBITED_GENERATORS.items():
            if signature in content:
                prohibited.append((Path(f).name, source_name))
                break
    ...
    print()
    print("✗ Cannot proceed — prohibited file(s) detected:")
    print()
    for filename, source_name in prohibited:
        print(f"  {filename}")
        print(f"    Generated by: {source_name}")
    print()
    print(
        "  Why this matters:\n"
        "  Services like UltraLibrarian, SnapMagic/SnapEDA, and SamacSys grant\n"
        "  you a license to use their files in your own designs only. They\n"
        "  explicitly prohibit redistribution in a public library or registry.\n"
        "  Contributing these files to HPM — even if you downloaded them\n"
        "  legally — would violate their terms of service and expose the project\n"
        "  to legal risk.\n"
        "\n"
        "  Note: DigiKey, Mouser, and Arrow link to these same services under\n"
        "  the hood. Files downloaded via their 'Download Symbol/Footprint'\n"
        "  buttons are typically from UltraLibrarian or SnapMagic.\n"
        "\n"
        "  Accepted alternatives:\n"
        "    • KiCad official library (CC-BY-SA-4.0)\n"
        "      Already installed with KiCad — search KiCad Symbol Editor or\n"
        "      Footprint Editor, or browse github.com/KiCad/kicad-symbols\n"
        "    • Drawn from the manufacturer's datasheet land pattern (CC0-1.0)\n"
        "      Most datasheets include recommended footprint dimensions on the\n"
        "      last pages under 'Package Outline' or 'PCB Land Pattern'\n"
        "    • Manufacturer's own download page\n"
        "      Some manufacturers (STMicroelectronics, NXP, etc.) host their\n"
        "      own KiCad files with permissive terms — check their site directly"
    )
    print()
    sys.exit(1)
    ...
    for attempt in range(1, 4):
        try:
            ...
            if "overloaded" in str(e).lower() and attempt < 3:
                wait = attempt * 15
                print(f"API overloaded — retrying in {wait}s (attempt {attempt}/3)...")
                time.sleep(wait)
            else:
...
def run_validate(json_files):
    cmd = [sys.executable, str(REPO_ROOT / "system_scripts" / "validate.py")] + [str(f) for f in json_files]
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "  python user_scripts/add_part.py --github your-handle"
        ),
    ...
    # Resolve file paths first — before any prompts — so we can validate files immediately.
    ...
    # Check files exist and scan for prohibited generators before asking anything else.
    validate_inputs(args)
    check_prohibited_files(args.footprint, args.symbol)
    ...
    try:
        ...
        datasheet_url = input("Datasheet PDF URL: ").strip()
    ...
    if validate_rc != 0:
        print(
            ...
            "Fix them and re-run:\n  python system_scripts/validate.py"
        )
